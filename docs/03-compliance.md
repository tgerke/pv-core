...
2. **Dev mode still exists.** `AUTH_MODE=dev` and the dev-grade role passwords (`pv_app`,
   ...
   `AUTH_MODE=oidc` and rotate the DB role credentials (docs/05-deployment.md); nothing in

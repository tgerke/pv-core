...
## ADR-0018: Docs screenshots and evaluator pages are generated from the seeded stack, never hand-made {#adr-0018}

`pnpm docs:screenshots` (`tools/screenshots.mjs`) is the only producer of the screenshots and generated artifacts under `site/src/assets/`. It drives headless Chrome against the dev stack, refuses to run unless the API is in dev auth mode and the database is exactly the seed, never mutates (dialogs opened and never confirmed, forms typed into and never submitted, every non-GET request blocked at the network layer), looks every entity up from the live API, and fails on seed drift. Regenerating is part of any commit that changes a pictured screen. The evaluator pages derive from existing prose: the fit assessment parses the roadmap's readiness table at build time and fails the build if it moves; the tour reuses sentences already on the user-guide, compliance, and roadmap pages. Feedback is collected privately, so the site produces a copyable summary rather than linking a public tracker.

## ADR-0019: Anticipated serious adverse events are a dated study list and a per-event sponsor designation; the reporting effect is a rule attribute {#adr-0019}

FDA's December 2025 IND safety reporting guidance separates *anticipated* (likely in the study population independent of the drug, §III.C) from *expected* (listed in the investigator's brochure) and says anticipated SAEs do not warrant individual IND safety reports; they are assessed in aggregate under 21 CFR 312.32(c)(1)(i)(C) (§IV.A.2.a), from a MedDRA-coded list in the safety surveillance plan (§V.A). Nothing in ICH E2A, E2B(R3), E2F, or Regulation (EU) 536/2014 has the concept, so it must not leak into their rules. The list is data (`study_anticipated_event` and its immutable terms, dated like an RSI version, a predicted rate never stored without its basis); the sponsor's judgment is data (`case_event_designation`, a separate child table so the reporter's facts stay untouched and versions hashed before it existed keep their hash; `assess` only, cloned, locked); the effect is `reporting_rule.excludes_anticipated`, set on the seeded FDA IND rules and never on the EU CTR ones, with `v_rule_anticipated_exclusion` naming every held-back rule. No rate is ever generated, and observed-versus-predicted monitoring is deferred until pv-core has an exposure denominator.

## ADR-0020: Investigator and sponsor causality assessments coexist; disagreement is derived, never adjudicated in the database {#adr-0020}

Both causality rows stay: the sponsor never edits the reporter's (Regulation (EU) 536/2014 Annex III §2.1 ¶4), both are exported (E2B(R3) IG §G.k.9.i) and shown in the DSUR (E2F §3.7.2(l)). A difference is a derived column, `causality_disagreement`, rolled up to the queue, the DSUR comment, and the digest; it is not a workflow state and there is no adjudication table. Which opinion starts a clock is the rule's `causality_basis`: `sponsor` on the seeded FDA IND rules (21 CFR 312.32; FDA Dec 2025 §IV.A), `either` on the EU CTR, investigator, and IRB rules (ICH E2A §III.A.1), with an unassessed sponsor still treated as related. Resolution with the site runs through the EDC query or correspondence, and the site's answer is follow-up information that opens a new version.
